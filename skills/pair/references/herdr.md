...
1. Run
   `node "$PAIR_SCRIPT" discover "${PAIR_ID[@]}"`. A partner pane already in
   ...
   ```bash
   node "$PAIR_SCRIPT" spawn "${PAIR_ID[@]}" --partner "$PARTNER" \
...
2. Run `node "$PAIR_SCRIPT" init "${PAIR_ID[@]}" [--role peer|executor]`.
